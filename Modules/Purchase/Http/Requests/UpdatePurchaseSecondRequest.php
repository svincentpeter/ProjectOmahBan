...
class UpdatePurchaseSecondRequest extends FormRequest
{
    /**
    ...
    {
        $purchaseSecondId = $this->route('purchaseSecond')->id ?? $this->route('purchaseSecond');

        return [
            // Main purchase info
            'date' => 'required|date',
            'reference' => ['required', 'string', 'max:255', Rule::unique('purchase_seconds', 'reference')->ignore($purchaseSecondId)],
