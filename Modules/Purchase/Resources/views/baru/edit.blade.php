            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 rounded-t-lg">
                    <h5 class="text-lg font-bold text-gray-800 dark:text-white mb-1">
                        <i class="cil-pencil mr-2 text-purple-600"></i> Edit Produk
                    </h5>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Tambah atau kurangi produk dalam pembelian ini.
                    </p>
                </div>
                <div class="p-6">
        ...
        <div class="lg:col-span-5 space-y-6">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 rounded-t-lg">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Perbarui informasi supplier dan pembayaran.
                    </p>
                </div>
                <div class="p-6">
                    <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchase_form">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <select id="payment_method" name="payment_method"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500"
                                    required>
                                    <option value="Cash" {{ $purchase->payment_method == 'Cash' || $purchase->payment_method == 'Tunai' ? 'selected' : '' }}>Cash/Tunai</option>
                                    <option value="Transfer" {{ $purchase->payment_method == 'Transfer' ? 'selected' : '' }}>Transfer</option>
                                    <option value="Credit" {{ $purchase->payment_method == 'Credit' ? 'selected' : '' }}>Credit/Hutang</option>
                                    <option value="Other" {{ $purchase->payment_method == 'Other' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                            </div>

                            {{-- Bank Name --}}
                            <div class="{{ $purchase->payment_method == 'Transfer' ? '' : 'hidden' }}" id="bank_name_div">
                                <label for="bank_name"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                    Nama Bank
                                </label>
                                <input type="text" name="bank_name" id="bank_name" value="{{ $purchase->bank_name }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500"
                            ...
                            <div>
                                ...
                                <textarea name="note" id="note" rows="3"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">{{ $purchase->note }}</textarea>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <input type="text" name="paid_amount" id="paid_amount" required value="{{ $purchase->paid_amount }}"
                                ...
                                <div class="flex justify-between items-center text-sm font-medium text-gray-600 dark:text-gray-400">
                                    ...
                                    <input type="hidden" name="due_amount" id="due_amount" value="{{ $purchase->due_amount }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8">
                            <button type="submit" id="btn_submit"
                                class="w-full text-white bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-3 text-center dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-800 transition-colors">
                                <i class="cil-save mr-2"></i> Update Pembelian
                            </button>
                        </div>
                    </form>
                </div>
        ...
        $(document).ready(function() {
            ...
            @foreach (Cart::instance('purchase')->content() as $item)
                cart.push({
                    ...
                    stock: parseInt('{{ $item->options->stock }}'),
            ...
            $('#paid_amount').maskMoney('mask'); // Apply initial mask
                    ...
                    tbody.html(`
                        <tr id="empty_cart">
                            <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                ...
                                Kosong (Tidak boleh kosong saat submit)
                            </td>
                        </tr>
                    `);
