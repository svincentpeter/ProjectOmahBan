...
class SaleController extends Controller
{
    ...
    public function show(Sale $sale)
    {
        ...
        // ✅ TAMBAHAN: Load data tracking harga di saleDetails
        $sale->load([
            'saleDetails' => function ($q) {
                $q->select(
                    'id',
                    ...
                    'product_discount_type',
                    'product_tax_amount',
                    // ✅ KOLOM BARU untuk tracking perubahan harga
                    'original_price',
                    'is_price_adjusted',
                    'price_adjustment_amount',
                    'price_adjustment_note',
            ...
            'salePayments',
            'user', // Kasir yang create
            // ✅ TAMBAHAN: Load user yang edit harga (via adjusted_by)
    ...
    public function update(UpdateSaleRequest $request, Sale $sale)
    {
        ...
        if ($items->isEmpty()) {
            return back()->with('swal-error', 'Keranjang kosong. Tambahkan item terlebih dahulu.')->withInput();
        }
        ...
        try {
            DB::transaction(function () use ($sale, $data, $items, $grand, $subtotalItems, $taxPct, $disPct, $ship, $paymentStatus, $existingPaid, $targetPaid) {
                // 3) Update header
                $sale->date = $data['date'];
            ...
            return back()
                ->with('swal-error', 'Gagal menyimpan perubahan: ' . $e->getMessage())
                ->withInput();
        }
