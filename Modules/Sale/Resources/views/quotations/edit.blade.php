            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
               <li class="inline-flex items-center">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <i class="bi bi-chevron-right text-gray-400"></i>
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Edit Penawaran</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <form id="quotation-form" action="{{ route('quotations.update', $quotation) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                         ...
                         <div class="relative">
                            ...
                            <input type="text" name="reference" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5 cursor-not-allowed" value="{{ $quotation->reference }}" readonly>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5" value="{{ $quotation->date }}" required>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select name="customer_id" id="customer_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full pl-10 p-2.5" required>
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ $quotation->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->customer_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="note" name="note" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500">{{ $quotation->note }}</textarea>
                </div>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none flex items-center shadow-lg hover:shadow-xl transition-all">
                        Update Quotation <i class="bi bi-check-lg ml-2"></i>
                    </button>
                </div>
            </form>
        </div>
