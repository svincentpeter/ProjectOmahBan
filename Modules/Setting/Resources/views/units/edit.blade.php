    ...
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="bi bi-chevron-right text-gray-400"></i>
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Ubah: {{ $unit->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                <div>
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">
                        <i class="bi bi-pencil-square mr-2 text-blue-600"></i>Ubah Satuan: {{ $unit->name }}
                    </h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui informasi satuan unit</p>
                </div>
                ...
                <div class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="short_name" name="short_name" value="{{ old('short_name', $unit->short_name) }}" 
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="note" name="note" rows="3" 
                                ...
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('note') border-red-500 @enderror">{{ old('note', $unit->note) }}</textarea>
                             @error('note')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="lg:col-span-1">
                ...
                <div class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <h4 class="mb-3 text-sm font-semibold text-gray-900 dark:text-white flex items-center">
                        <i class="bi bi-clock-history text-gray-500 mr-2"></i> Riwayat Perubahan
                    </h4>
                    <dl class="space-y-2 text-sm text-gray-900 dark:text-white">
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Dibuat pada:</dt>
                            <dd class="font-medium">{{ $unit->created_at->format('d M Y H:i') }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500 dark:text-gray-400">Terakhir diubah:</dt>
                            <dd class="font-medium">{{ $unit->updated_at->format('d M Y H:i') }}</dd>
                        </div>
                    </dl>
                </div>

                {{-- Tips Card --}}
                 <div class="p-4 mb-4 bg-white border-l-4 border-yellow-400 rounded-lg shadow-sm dark:bg-gray-800 dark:border-yellow-600">
                     <div class="flex items-start">
                         <div class="flex-shrink-0">
                             <i class="bi bi-lightbulb text-xl text-yellow-500 dark:text-yellow-400"></i>
                         </div>
                         <div class="ml-3">
                             <h3 class="text-lg font-medium text-yellow-800 dark:text-yellow-300">Tips</h3>
                             <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-200">
                                 <p>Mengubah satuan akan mempengaruhi tampilan pada produk yang menggunakan satuan ini. Pastikan perubahan tidak mengubah makna.</p>
                             </div>
                         </div>
                     </div>
                 </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Char counter initialization and update
        function updateCharCounter() {
            const currentLength = shortNameInput.value.length;
        ...
        shortNameInput.addEventListener('input', updateCharCounter);
        updateCharCounter(); // Run once on load
