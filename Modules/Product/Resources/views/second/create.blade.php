...
<div class="container-fluid">
    <div class="animated fadeIn">
        ...
        <form id="product-form" action="{{ route('products_second.store') }}" method="POST" enctype="multipart/form-data">
            ...
            {{-- Include Modern Form (without $product for create mode) --}}
            @include('product::second._form')
        </form>
    </div>
</div>
@endsection
...
$(document).ready(function() {
    ...
    var myDropzone = new Dropzone("#document-dropzone", {
        ...
        error: function(file, message) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Upload Gagal',
                text: typeof message === 'string' ? message : 'Terjadi kesalahan saat upload',
    ...
    function calculateProfit() {
        ...
        if (purchase > 0 && selling > 0) {
            ...
            // Color coding
            if (percentage < 10) {
                $('#profitPercentage').removeClass('badge-primary badge-success badge-warning').addClass('badge-danger');
            } else if (percentage < 30) {
                $('#profitPercentage').removeClass('badge-primary badge-danger badge-success').addClass('badge-warning');
            } else {
